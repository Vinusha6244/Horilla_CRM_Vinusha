...
public class Lead extends BaseClass{
	...
	{   driver.manage().window().maximize();
        ...
        Thread.sleep(2000);
        UtilityClassObject.getTest().log(Status.PASS, "Lead Created succefully and confirmation message displayed Succecfully");
        salesPage.getLeadsLink().click(); 
        
        List<WebElement> actualOpportunityName= driver.findElements(By.xpath("//tbody[@id='data-container-leads-list']//td/div[contains(text(),'"+title+"')]"));
		  for(WebElement ele:actualOpportunityName) 
		  {
			  String filterName=ele.getText();
		  if(filterName.equals(title))
		  {   
			  UtilityClassObject.getTest().log(Status.PASS, "Lead created successfully! and Succecfully displayed in Lead List page");
			  ele.click();
			  break;
		  }
		  }
